...
class ESPectreGame {
    ...
    parseGameData(line) {
        // Format: [I][stream:NNN][wifi]: <movement>,<threshold>
        // Example: [I][stream:075][wifi]: 0.08,1.00
    ...
    updateMovement(value) {
        ...
        // Calculate display scale: threshold should be at ~50% of the bar
        // This way we can see both below and above threshold movements
        const maxDisplay = this.threshold * 2;
        const displayPercent = Math.min(100, (this.movement / maxDisplay) * 100);
        const thresholdPercent = Math.min(100, (this.threshold / maxDisplay) * 100);
        ...
        if (this.elements.movementThresholdMarker) {
            this.elements.movementThresholdMarker.style.left = thresholdPercent + '%';
        }
